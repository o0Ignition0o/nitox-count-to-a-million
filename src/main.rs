...
fn handle_second_message_stream(
    message_stream: impl Stream<Item = Message, Error = NatsError> + Send + 'static,
) -> impl Future<Item = (), Error = NatsError> {
    tokio::spawn({
        connect_to_nats().map_err(|_| ()).and_then(|client| {
            message_stream
                .for_each(move |msg| match std::str::from_utf8(&msg.payload) {
                    Ok(payload) => {
                        println!("Received {} on the second race channel", payload);
                        // Publish to an other channel
                        let response_command = PubCommand::builder()
                            .subject("Race")
                            .payload("123")
                            .build()
                            .unwrap();
                        let _ = client.publish(response_command);
                        ok(())
                    }
                    Err(_) => {
                        println!(
...
fn subscribe_to_second_race(
    client: NatsClient,
) -> impl Future<Item = NatsClient, Error = NatsError> {
    client
        .subscribe(SubCommand::builder().subject("Race2").build().unwrap())
        .and_then(move |message_stream| handle_second_message_stream(message_stream))
        .and_then(|_| ok(client))
}

fn publish_to_second_race(
    client: NatsClient,
    payload: String,
) -> impl Future<Item = NatsClient, Error = NatsError> {
    client
...
fn get_tasks_to_perform() -> impl Future<Item = (), Error = ()> {
    connect_to_nats()
        ...
        .and_then(subscribe_to_second_race)
        .and_then(|client| publish_to_race(client, "42".into()))
        .and_then(|client| publish_to_second_race(client, "HELLO".into()))
